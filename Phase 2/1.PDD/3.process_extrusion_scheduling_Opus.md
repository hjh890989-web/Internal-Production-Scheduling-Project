...
| BR-E11 | PDD-02가 변경되면 본 프로세스는 자동으로 트리거 — 수동 카톡 전달 금지. **(v1.1 주석)** PDD-02 BR-V12(capa 초과 추가요청 채택분) 및 BR-V13(KD 보충분) 활성 시에도 동일 cascade 적용 — 별도 룰 신설 없이 기존 트리거가 흡수 | T9.1 | 마스터 문제정의서 §3 단절 3 + PDD-02 v1.1 동기 |
...
| `Phase 2/1.PDD/2.process_vulcanization_scheduling_Opus.md` v1.1 | 상류 PDD (성형 투입일 입력원). v1.1에서 BR-V07 당일 락 + BR-V12~17 신규 7건 — 본 PDD-03은 BR-V17 cross-reference(BR-E12) 외 룰 변경 없음 |
| `Phase 2/3.SAD/SAD-001_Production_Scheduling_System_v1.0.md` v1.1 (ADR-017) | 성형↔압출 cross-master VIEW `v_product_with_spec` + Caffeine 캐시 무효화 패턴 |
