...
c) **앵글 교체 횟수가 최소화**된다 — 동일 앵글 사용 제품을 인접 회전에 묶어 배치한다.
...
| (외부) 제약 마스터 | ← 참조 | 성형공정_제약조건.xlsx (품번별 슬롯 O/X·합금형·앵글) |
...
flowchart TD
    ...
    subgraph IN_TB["Pool: 사내"]
        subgraph SYS_TB["Lane: 스케줄링 엔진 (L4)"]
            ...
            T5["Service Task:<br>앵글 그룹핑·<br>교체 최소화"]:::svc
            G_VAL{"제약<br>충족?"}:::gw
...
| T4.5 | `T4` | The system **shall** check **angle availability**: `total slots assigned to 품번 ≤ 앵글보유수량 (F or L)`. | L4 | 앵글 capa 검증 |
...
| BR-V06 | 앵글 보유수량(F, L)을 동시에 점유하는 슬롯 수가 초과하면 안 됨 | T4.5 | 클로드_성형_프롬프트.docx |
...
| K-V02 | 일일 앵글 교체 횟수 (슬롯당 평균) | **5~6회** | **≤3회** | DO-04 집계 | 주 |
...
- [ ] **c) 앵글 교체 최소** — 동일 데이터셋으로 그리디 베이스라인 대비 평균 ≥30% 감소
...
| c) 앵글 교체 최소 | `7.persona_pain_goal_analysis.md` P2 GAP=3, JTBD DOS=2.0 | P2 | SRS-FR-VC-030 |
...
| R-V03 | 앵글 교체 최적화로 인한 현장 동선 악화 | 저 | 🟡 | L3 시뮬레이션 뷰에서 사전 검증, 사용자 조정 허용 |
...
| `Phase 1/2.Raw Materials/Vulcanization/성형공정_제약조건.xlsx` | 슬롯 O/X · 합금형 · 앵글 데이터 원본 |
