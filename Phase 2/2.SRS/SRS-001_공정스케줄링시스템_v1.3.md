...
| ID | 문서 | 위치 |
|----|------|------|
| REF-01 | PDD + PRD 통합 마스터 v1.3 | `Phase 2/1.PDD/4.PDD_master_integrated_Opus.md` |
...
| REQ-FUNC-VC-007 | 앵글 가용량 제약 | 시스템은 hose_id를 동시 점유하는 슬롯 총 수가 앵글 보유량(`lp_angle_qty` 또는 `ic_angle_qty`)을 초과하지 않도록 보장해야 한다 | US-02 / M-05 / BR-V06 | Must | 의도적 stress 입력 회귀에서 앵글 과초과 점유 0건 |
...
| REQ-FUNC-VC-012 | 앵글 교체 그룹핑 최적화 | 시스템은 한 슬롯에서 동일 앵글을 사용하는 연속 회전을 그룹핑하여 앵글 교체 이벤트를 최소화해야 한다 | US-03 AC-2 / S-01 | Should | 그리디 베이스라인 대비 회귀에서 ≥30% 교체 감소 |
...
> 기능 요구사항 합계: **OC 15 + VC 20 + EX 20 + CO 10 + XT 3 = 68개**.
...
| REQ-FUNC-VC-012 | 그리디 베이스라인 -30% | A | T-I | REF-01 §16 US-03 AC-2 |
...
| REQ-NF-SEC-007 | 비밀번호 정책 | 최소 12자, 3종 문자 클래스, 5회 실패 시 잠금 | derived | Should | 정책 리뷰 |
...
| REQ-NF-KPI-007 | K-V02 — 일·슬롯당 평균 앵글 교체 | 5–6 | ≤3 | 주 | K-V02 |
...
> 비기능 요구사항 합계: **PER 8 + REL 6 + SEC 7 + USA 5 + OPS 7 + COM 5 + COS 3 + KPI 19 = 60개**.
...
| EXP-3 | 시스템이 수동 계획 대비 앵글 교체를 감소시킨다 | 4주 계획의 A/B 비교 | 시스템 ≥30% 적은 교체 |
