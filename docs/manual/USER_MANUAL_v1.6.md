# 사내 공정 스케줄링 시스템 — 사용자 매뉴얼 v1.6

**대상**: 송우산업 베타 사용자 8명 (PLANNER 3 + STK_USER 3 + IT_OPS 1 + READ_ONLY 1)
**작성일**: 2026-06-01 | **버전**: 1.6 | **참조**: Sprint 26 S26-A ST-DOC-1 (v1.5 Sprint 25 S25-A ST-PROD-3 기반)

> 본 매뉴얼은 베타 운영 진입 시점의 4 role 별 표준 작업 절차(SOP). 시스템 변경 시 새 버전(v1.x+) 으로 분리 발행. 스크린샷은 `docs/manual/screenshots/` 폴더 별도.
> **v1.6 변경**: §1.2 Diff 검토 + 확정 — Sprint 26 Order 자동 INSERT chain (config flag `scheduling.order.auto-draft.enabled`, default false) 안내 추가. §3.6.6 Phase 5+ carry-over 에 Sprint 26 S26-B vendor mapper placeholder (사내 IT vendor spec 수신 후 mock contract → 실 vendor DTO 전환 + WireMockMesIT 4 시나리오 갱신 + Phase 5+ EP-MQ-ADAPTER chain) 추가.
> **v1.5 누적**: §3.7 IT_OPS Blue/Green 무중단 배포 가이드 — Sprint 25 ST-PROD-3 활성. §3.7.1 배포 mode 선택 (수동 blue-green-switch.sh vs 자동 blue_green_deploy.sh Jenkins) + §3.7.2 첫 배포 전 green 사전 기동 (1회) + §3.7.3 정상 절차 5단계 (Harbor pull → switch → health → smoke → Grafana) + §3.7.4 rollback 3 시나리오 (auto / 수동 / missing 복구) + §3.7.5 트러블슈팅 4건 (NGINX reload / image pull / DB migration / healthcheck timeout) + §3.7.6 Grafana 배포 dashboard cross-ref (Sprint 21 deployment marker). §6 장애 대응 표에 `Blue/Green deploy 실패` 1줄 추가 — §3.7.4 rollback 참조만 (중복 회피).
...
**Sprint 26 신규 — 자동 INSERT chain (config flag default false)**: PLANNER `확정` 후 OrderCommittedEvent 발행 → 자동 chain 활성 시 VC 스케줄 draft 자동 생성. config flag `scheduling.order.auto-draft.enabled=true` (env `ORDER_AUTO_DRAFT_ENABLED=true`) 활성 후 동작. default false 유지 시 기존 PLANNER manual draft 흐름 유지. BR-X02 audit — actor=system + reason=auto-chain-from-order-commit.
...
**Sprint 26 S26-B carry-over (사내 IT vendor spec 수신 후)**: mock contract DTO (MesShiftResponse 5 필드) → 실 vendor DTO 전환 (필드 추가/제거/이름 변경 / endpoint 경로 / auth 방식 / pagination / error format). mapper 작성 후 WireMockMesIT 4 시나리오 갱신 + Phase 5+ EP-MQ-ADAPTER 와 chain.
...
| **1.6** | **2026-06-01** | **Claude Code (Sprint 26 ST-DOC-1)** | **§1.2 Order 자동 INSERT chain 안내 (config flag default false). §3.6.6 vendor mapper placeholder (S26-B carry-over).** |

> **버전 주석**: v1.1 은 Sprint 20 EP-EXT-WEBHOOK (Slack/Kakao webhook, S21 와 병렬) 의 §6 장애 대응 addendum 으로 예약. S21 (마스터 운영) → v1.2, S23 (MES adapter) → v1.3, S24 ST-FB-3 (Grafana alert) → v1.4, S25 ST-PROD-3 (Blue/Green deploy) → v1.5, S26 ST-DOC-1 (Order 자동 chain + vendor mapper carry-over) → v1.6 순으로 마감. Sprint 20 마감 후 v1.7 합본 검토.
