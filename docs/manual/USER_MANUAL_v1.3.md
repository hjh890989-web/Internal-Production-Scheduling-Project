...
4. 첫 polling (60초 후) 결과 검증:
   - Grafana → Application Overview → MES polling success rate (Sprint 24 ST-FB-3 추가 예정)
...
- Grafana 패널 — MES polling success rate / circuit state visualization (Sprint 24 ST-FB-3 예정)
...
> **버전 주석**: v1.1 은 Sprint 20 EP-EXT-WEBHOOK (Slack/Kakao webhook, S21 와 병렬) 의 §6 장애 대응 addendum 으로 예약. S21 (마스터 운영) 이 먼저 마감되어 v1.2 로 발행. S23 (MES adapter) 가 다음으로 마감되어 v1.3 으로 발행. Sprint 20 마감 후 v1.4 합본 검토.
